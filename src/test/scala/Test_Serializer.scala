...
import xyz.bitml.api.{ChunkEntry, ChunkType, Serializer, TxEntry, TxStorage}
...
class Test_Serializer extends AnyFunSuite {
  ...
  test ("Serialization of TxEntry with chunk data.") {
    ...
    println(serialized)
    println(ser.deserializeTxEntry(serialized))
  }

  /*
  test ("Deserialization of JSON string with no chunk data") {
    println(new Serializer().deserializeTxEntry(""))
  }
